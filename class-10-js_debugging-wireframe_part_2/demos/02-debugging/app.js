...
Store.prototype.generateRandomCustPerHour = function(min, max) {
  for (var i = 0; i < thishoursOfOps.length; i++) {
...
Store.prototype.render = function() {
  ...
  trEl.id = this.name.toLowerCase().split(' ').join("_"); // Scott will come back to this if we have time.
...
function createTableFooter() {
  ...
  for(var i = 0; i < hoursOfOps.length; i++) {
    ...
    for(var j = 0; j < stores.length; j++) {
      totals += stores[j].cookiesPerHour[j];
    }
